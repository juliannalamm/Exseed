...
with tab2:
    ...
    st.markdown("**Select from Example Data**")
    
    # Check if example_data folder exists
    example_data_dir = "example_data"
    if os.path.exists(example_data_dir):
        # Get list of participant folders
        participant_folders = [f for f in os.listdir(example_data_dir) if os.path.isdir(os.path.join(example_data_dir, f))]
        ...
        if participant_folders:
            ...
            if selected_participant:
                participant_dir = os.path.join(example_data_dir, selected_participant)
                json_files = [f for f in os.listdir(participant_dir) if f.endswith('.json')]
                video_files = [f for f in os.listdir(participant_dir) if f.endswith('.mp4')]
                ...
                # Set file paths
                if selected_json and selected_video:
                    json_path = os.path.join(participant_dir, selected_json)
                    video_path = os.path.join(participant_dir, selected_video)
                    json_file = True  # Flag to indicate file is selected
                    video_file = True
                else:
                    json_file = None
                    video_file = None
            ...
            st.error("❌ No participant folders found in example_data directory.")
            json_file = None
            video_file = None
            run_btn = False
    else:
        st.error("❌ example_data folder not found. Please create an 'example_data' folder with participant subfolders containing JSON and MP4 files.")
        json_file = None
        video_file = None
        run_btn = False
    ...
    if run_btn and json_file and video_file:
        with st.spinner("Processing your data..."):
            # Use the selected file paths directly
            participant_id = os.path.splitext(os.path.basename(selected_json))[0]
    ...
    if 'preds' in st.session_state and 'frame_df' in st.session_state:
        ...
        st.error("❌ Please select both JSON and video files to run analysis.")
